...
class NumMatrix(object):
    ...
    def __init__(self, matrix):
        ...
        self.cache = [[0 for _ in range(col + 1)] for _ in range(row + 1)]
        for i in range(0, row):
            for j in range(0, col):
                self.cache[i + 1][j + 1] = self.cache[i][j + 1] + self.cache[i + 1][j] - self.cache[i][j] + matrix[i][j]
